...
async def get_ibkr_data_async(
    ...
    try:
        ...
        start_date_dt = datetime.strptime(start_date, "%Y%m%d") # Handle case where start_date is today
    try:
        end_date_dt = datetime.strptime(end_date, "%Y-%m-%d").replace(hour=23, minute=59, second=59) # Include full end day
    except ValueError as e:
        end_date_dt = datetime.strptime(end_date, "%Y%m%d") # Handle case where start_date is today
