...
Complete curriculum assessment and next steps planning
"""
...
class Phase2Analyzer:
    ...
    def analyze_curriculum_phases(self):
        ...
        for phase_name, (start, end) in phases.items():
            ...
            results[phase_name] = {
                ...
                'avg_ep_len': np.mean(phase_ep_lens) if phase_ep_lens else 0,
                'avg_ep_rew': np.mean(phase_ep_rews) if phase_ep_rews else 0,
    ...
    def analyze_tax_effectiveness(self):
        ...
        print(f"   Average penalized episode length: {np.mean(self.data['tax_episodes']):.1f} steps")
        
        # Analyze tax frequency over time
        ...
        return {
            'tax_count': tax_count,
            'tax_rate': tax_rate,
            'avg_penalized_length': np.mean(self.data['tax_episodes']) if self.data['tax_episodes'] else 0
        }
    ...
    def analyze_drawdown_behavior(self):
        ...
        dd_stats = {
            ...
            'avg_dd': np.mean(self.data['dd_terminations']),
